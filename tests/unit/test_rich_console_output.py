...
from models import LunaResults, LunaShortcutConfig
...
class TestRichOutputSnapshots:
    ...
    def test_completion_summary_snapshot(self, rich_snapshot_console, snapshot_tester):
        ...
        results = LunaResults()
        results.directories_created = [Path("/test/Luna"), Path("/test/Luna/config")]
        results.exclusions_added = [(Path("/test/Luna"), True), (Path("/test/Luna/config"), True)]
        results.files_extracted = True
        results.mark_extraction_attempted()
