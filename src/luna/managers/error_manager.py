...
class ErrorManager:
    ...
    def _build_error_registry(self) -> Dict[Type[Exception], ErrorCategory]:
        ...
        return {
            ...
            PlatformNotSupportedError: ErrorCategory.PLATFORM,
            
            # Luna-specific errors
            LunaError: ErrorCategory.RECOVERABLE,
            LunaMigrationError: ErrorCategory.RECOVERABLE,
            LunaConfigurationError: ErrorCategory.RECOVERABLE,
            LunaComponentError: ErrorCategory.RECOVERABLE
        }
    ...
    def _build_solution_registry(self) -> Dict[Type[Exception], str]:
        ...
        return {
            ...
            PlatformNotSupportedError: "This platform is not supported for this operation",
            
            # Luna-specific errors
            LunaError: "Check Luna installation and configuration files",
            LunaMigrationError: "Verify legacy installations exist and are accessible, or skip migration",
            LunaConfigurationError: "Check Luna configuration files for errors or reset to defaults",
            LunaComponentError: "Verify Luna components are properly installed and not blocked by antivirus"
        }
    ...
    def get_luna_solution(self, error: LunaError) -> Optional[str]:
        """Get Luna-specific solution for errors.
        
        Args:
            error: Luna-specific exception to get solution for
        ...
        # Fallback to generic Luna solution
        return "Check Luna installation and configuration files"
    ...
    def display_luna_error(self, error: Exception, context: str) -> None:
        ...
        Args:
            ...
            context: Context where the error occurred
        """
        category = self.categorize_luna_error(error)
        solution = self.get_luna_solution(error) if isinstance(error, LunaError) else self.get_solution(error)
        
        error_text = Text()
        error_text.append("Luna Gaming Tool - Error Report\n", style="bold magenta")
        error_text.append(f"Context: {context}\n", style="bold")
        error_text.append(f"Error: {str(error)}\n", style="red")
        error_text.append(f"Category: {category.value.capitalize()}\n", style="yellow")
        ...
        panel = Panel(
            error_text,
            title=f"Luna {category.value.capitalize()} Error",
            **PANEL_STYLES['error']
        )
        ...
        # Log the error with appropriate level based on category
        if category == ErrorCategory.CRITICAL:
            self.logger.critical(f"Luna - {context}: {str(error)}")
        elif category == ErrorCategory.NETWORK:
            self.logger.warning(f"Luna network error in {context}: {str(error)}")
        else:
            self.logger.error(f"Luna - {context}: {str(error)}")
    
    def categorize_luna_error(self, error: Exception) -> ErrorCategory:
        """Categorize Luna error with Luna-specific logic.
        ...
        Returns:
            ErrorCategory for the given exception
        """
        # Handle Luna-specific errors first
        if isinstance(error, LunaMigrationError):
            return ErrorCategory.RECOVERABLE
        elif isinstance(error, LunaConfigurationError):
            return ErrorCategory.RECOVERABLE
        elif isinstance(error, LunaComponentError):
            return ErrorCategory.RECOVERABLE
        elif isinstance(error, LunaError):
            return ErrorCategory.RECOVERABLE
        
        # Fall back to standard categorization
        return self.categorize_error(error)
...
def create_user_friendly_message(error: Exception, context: str) -> str:
    ...
    friendly_messages = {
        ...
        # Luna-specific error messages
        "LunaError": "A Luna Gaming Tool error occurred",
        "LunaMigrationError": "Luna migration from legacy installations failed",
        "LunaConfigurationError": "Luna configuration error occurred",
        "LunaComponentError": "Luna component operation failed"
    }
